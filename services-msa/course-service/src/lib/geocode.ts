...
// 쓰는 헬퍼였다. 폐쇄망 전환으로 외부 API 호출이 막히면서 이 함수들을 부르던 같은 디렉터리의
// routes/geo.ts /geo/dong-center, /geo/reverse-dong 라우트를 같은 날짜에 비활성화했다(그 파일의
// 주석 참고). 이제 이 파일을 호출하는 곳이 없다. 재활성화하려면 법정동 경계/중심좌표 데이터를
// 로컬 PostGIS 테이블로 적재하는 방식으로 다시 구현해야 한다(외부 API로 되돌리지 않는다).
//
