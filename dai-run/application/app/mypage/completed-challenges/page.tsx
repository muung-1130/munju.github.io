import Link from 'next/link';
import { getServerSession } from 'next-auth';
import { authOptions } from '@/lib/auth';
import { CompletedChallengesSection } from '@/components/CompletedChallengesSection';
import { getCompletedChallenges } from '@/lib/challenges';

export const dynamic = 'force-dynamic';

export default async function CompletedChallengesPage() {
  const session = await getServerSession(authOptions);
  const userId = session?.user?.id;
  const challenges = userId ? await getCompletedChallenges(userId) : [];

  return (
    <div className="mypage-subpage">
      <Link href="/mypage" className="mypage-back-link">← 마이페이지</Link>
      <CompletedChallengesSection challenges={challenges} />
    </div>
  );
}
