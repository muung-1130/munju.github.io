...
export function parseSqsDomainEvent(body: string, allowedEventTypes: readonly string[]): SqsDomainEvent {
  ...
  if (isRecord(parsed)) {
    ...
    if (notification.Type === 'Notification' && typeof notification.Message === 'string') parsed = JSON.parse(notification.Message);
  }
  if (!isRecord(parsed) || typeof parsed.eventId !== 'string' || !parsed.eventId) throw new Error('유효한 eventId가 없습니다.');
  ...
  if (!isRecord(parsed.payload)) throw new Error('이벤트 payload가 객체가 아닙니다.');
  ...
  return {
    async start(): Promise<void> {
      ...
      for (;;) {
        let messages;
        try {
          const response = await client.send(new ReceiveMessageCommand({ QueueUrl: options.queueUrl, MaxNumberOfMessages: 1, WaitTimeSeconds: WAIT_TIME_SECONDS }));
