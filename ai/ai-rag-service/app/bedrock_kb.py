...
class BedrockKnowledgeBaseClient:
    ...
    def today_coaching(
        ...
        prompt = (
            ...
            f"오늘의 수준별 코칭 기준:\n{level_guidance}\n\n"
            "실제 코치와 채팅하듯 친근한 존댓말로 답하세요.\n"
            "짧은 도입 1~2문장, 하이픈 목록 3개 이내, 다음 행동을 묻는 짧은 질문 하나로 구성하세요.\n"
            "각 목록에는 거리 또는 시간, 강도, 진행 방법 중 필요한 정보를 한 문장으로 담으세요.\n"
            "**오늘의 추천** 같은 고정 제목은 가독성에 꼭 필요할 때만 사용하세요.\n"
            "답변은 500자 이내로 작성하고 번호 목록, 중첩 목록, 장황한 서론, 반복은 사용하지 마세요.\n"
        ...
        if self._is_refusal(answer):
            retry_prompt = (
                ...
                "짧은 안내 후 거리 또는 시간, 강도, 진행 방법을 하이픈 목록 3개 이내로 "
                "작성하고 후속 질문 하나로 마무리하세요. 의료 진단이나 처방은 하지 마세요."
            )
            retry_request = dict(request)
    ...
    def _build_chat_prompt(
        ...
        if concise:
            prompt_parts.append(
                "사용자의 러닝 질문에 직접 답하세요. 친근한 한국어로 짧게 설명하고, "
                "필요하면 하이픈 목록을 사용한 뒤 후속 질문 하나로 마무리하세요."
            )
        else:
            prompt_parts.append(
                "답변 방식:\n"
                "- 친근하고 자연스러운 존댓말을 사용하세요.\n"
                "- 질문이 넓고 핵심 조건이 부족하면 가장 중요한 확인 질문 하나와 선택지 2~4개만 제시하세요.\n"
                "- 구체적인 질문에는 도입 1~2문장, 하이픈 목록 최대 5개, 후속 질문 하나 순서로 답하세요.\n"
                "- 후속 대화에는 앞 내용을 반복하지 말고 2~4문장으로 바로 답하세요.\n"
                "- 여러 주의 계획은 목적이 같은 2~4개 구간으로 묶으세요.\n"
                "- 실제 날씨 수치가 있으면 일반론 대신 그 수치를 사용하세요.\n"
                "- 700자 이내로 작성하고 진단이나 처방은 하지 마세요."
            )
