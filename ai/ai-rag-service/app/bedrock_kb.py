...
class BedrockKnowledgeBaseClient:
    ...
    def today_coaching(
        ...
        prompt = (
            ...
            f"오늘의 수준별 코칭 기준:\n{level_guidance}\n\n"
            "실제 코치와 채팅하듯 친근한 존댓말로 답하세요. 마크다운 기호(**, #, -)나 줄바꿈에 "
            "기대지 말고 이어지는 문장으로 답하세요 — 화면이 마크다운을 그대로 텍스트로 보여주고 "
            "줄바꿈도 사라집니다. 짧은 도입 한 문장 뒤 거리 또는 시간·강도·진행 방법을 문장으로 "
            "자연스럽게 엮어 설명하고, 다음 행동을 묻는 짧은 질문 하나로 마무리하세요. "
            "답변은 500자 이내로 작성하고 장황한 서론이나 반복은 사용하지 마세요. "
        ...
        if self._is_refusal(answer):
            retry_prompt = (
                ...
                "마크다운 기호나 줄바꿈 없이, 짧은 안내 후 거리 또는 시간·강도·진행 방법을 "
                "이어지는 문장으로 설명하고 후속 질문 하나로 마무리하세요. 의료 진단이나 "
                "처방은 하지 마세요."
            )
            retry_request = dict(request)
    ...
    def _build_chat_prompt(
        ...
        if concise:
            prompt_parts.append(
                ...
                "마크다운 기호(**, #, -)나 줄바꿈에 기대지 말고 이어지는 문장으로 답한 뒤 "
                "후속 질문 하나로 마무리하세요."
            )
        else:
            prompt_parts.append(
                "답변 방식: 친근하고 자연스러운 존댓말을 쓰세요. 마크다운 기호(**, #, -)나 "
                "줄바꿈에 기대지 말고 이어지는 문장으로 답하세요 — 화면이 마크다운을 그대로 "
                "텍스트로 보여주고 줄바꿈도 사라집니다. 항목이 여러 개면 '첫째, ... 둘째, ...'처럼 "
                "문장 안에서 순서를 붙이거나 쉼표로 자연스럽게 나열하세요. 질문이 넓고 핵심 조건이 "
                "부족하면 가장 중요한 확인 질문 하나와 선택지 2~4개를 문장으로 풀어서 제시하세요. "
                "구체적인 질문에는 도입 한 문장 뒤 핵심 내용을 2~4문장으로 풀고 후속 질문 하나로 "
                "마무리하세요. 후속 대화에는 앞 내용을 반복하지 말고 2~4문장으로 바로 답하세요. "
                "여러 주의 계획은 목적이 같은 2~4개 구간으로 묶어 문장으로 설명하세요. 실제 날씨 "
                "수치가 있으면 일반론 대신 그 수치를 사용하세요. 500자 이내로 작성하고 진단이나 "
                "처방은 하지 마세요."
            )
